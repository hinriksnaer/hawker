#!/usr/bin/env fish
# Set Yazi flavor based on Hawker theme
...
else if test -f "$HOME/.local/share/hawker/modules/yazi/config/.config/yazi/theme-map.conf"
    set theme_map "$HOME/.local/share/hawker/modules/yazi/config/.config/yazi/theme-map.conf"
else
...
printf '%s\n' \
    ...
    '# This file automatically switches flavors based on the active Hawker theme' \
    '' \
    '[flavor]' \
    "use = \"$yazi_flavor\"" \
    > $theme_file
