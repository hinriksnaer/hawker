...
if test -d "$hooks_dir"
    for hook_file in $hooks_dir/*
        ...
        # Handle config-rewrite type (rewrites a JSON key in a config file)
        if test "$hook_type" = "config-rewrite"
        ...
        if test "$hook_type" = "hyprland"
            ...
            if test -f "$theme_path/hyprland.conf"
                ...
                printf '%s\n' \
                    ...
                    '# Managed by hawker-theme-set-desktop' \
                    "# theme: $theme_name" \
                    '' \
                    > $active_theme_conf
